...
const placenow = async (req, res) => {
  const orderdata = req.body.orderdata;
  const stockDetail = req.body.stockDetail;
  const products_name = stockDetail.name;
  const products_qty = stockDetail.qty;
  try {
    await ordercollection.insertMany([orderdata])
    // Stock managing
    try {
    ...
    console.log(error.message);
    console.log("error in placing online order");
  }
}
